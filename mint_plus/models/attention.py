...
class MultiHeadAttention(nn.Module, IncrementalStateMixin):
    ...
    def __init__(
        ...
        if use_fused_qkv:
            # Single fused projection for Q/K/V: one Linear(embed_dim, 3*embed_dim)
            # chunk(3) splits into q, k, v. Fewer kernel launches than 3 separate.
            self.fused_qkv = nn.Linear(embed_dim, 3 * embed_dim, bias=bias)
            self.q_proj = None
            self.k_proj = None
            self.v_proj = None
        else:
    ...
    def _init_parameters(self):
        ...
        if self.use_fused_qkv:
            nn.init.xavier_uniform_(self.fused_qkv.weight, gain=gain)
            if self.fused_qkv.bias is not None:
                nn.init.zeros_(self.fused_qkv.bias)
        else:
    ...
    def _project_qkv(self, x: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        """Project Q, K, V from input x. Handles both fused and separate paths."""
        if self.use_fused_qkv:
            qkv = self.fused_qkv(x)
            q, k, v = qkv.chunk(3, dim=-1)  # each (T, B, E), views of qkv
            # Make contiguous for view/reshape in downstream methods
            return q.contiguous(), k.contiguous(), v.contiguous()
        else:
            return self.q_proj(x), self.k_proj(x), self.v_proj(x)

    def project_qkv_4d(self, x: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        """Project Q, K, V and return as contiguous 4D (B, H, T, D) tensors.

        Skipped: scaling, RoPE, incremental_state, need_weights branching.
        This is a minimal projection path for use by the fused multi-pathway
        attention kernel. The caller must apply scaling and RoPE separately.

        Args:
            x: (T, B, E)

        Returns:
            q, k, v: each (B, H, T, D) contiguous bf16
        """
        T, B, E = x.shape
        H, D = self.num_heads, self.head_dim
        q, k, v = self._project_qkv(x)
        # View as (T, B, H, D) -> permute to (B, H, T, D) -> contiguous for kernel
        q = q.view(T, B, H, D).permute(1, 2, 0, 3).contiguous()
        k = k.view(T, B, H, D).permute(1, 2, 0, 3).contiguous()
        v = v.view(T, B, H, D).permute(1, 2, 0, 3).contiguous()
        return q, k, v

    def forward_before_softmax(
        self,
        x: Tensor,
        key_padding_mask: Optional[Tensor] = None,
    ) -> Tuple[Tensor, Tensor]:
        """Fast path for before_softmax — no branching, no extra copies.

        Skips incremental_state, need_weights, attn_mask branching.
        Only does: QKV proj -> flatten -> RoPE -> bmm -> padding_mask.
        ...
        Returns:
            logits: (B, H, T, T) raw attention logits
            v_heads: (B, H, T, D) value vectors
        """
        T, B, E = x.shape
        H, D = self.num_heads, self.head_dim

        q, k, v = self._project_qkv(x)
        q = q * self.scaling   # (T, B, E)

        # Flatten heads: (T, B, E) -> (B*H, T, D) with contiguous copy
        q = q.view(T, B * H, D).transpose(0, 1).contiguous()
        k = k.view(T, B * H, D).transpose(0, 1).contiguous()
        v_flat = v.view(T, B * H, D).transpose(0, 1).contiguous()
        ...
        if key_padding_mask is not None:
            logits = logits.view(B, H, T, T)
            logits = logits.masked_fill(
                key_padding_mask[:, None, None, :], float("-inf")
            )
            logits = logits.view(B * H, T, T)

        return logits.view(B, H, T, T), v_flat.view(B, H, T, D)
...
class MultimerAttention(nn.Module):
    """Dedicated cross-chain attention for the multimer pathway.

    Stripped-down version of MultiHeadAttention that only supports the
    before_softmax path used by the multimer combine. Features:

      - Fused QKV projection (single Linear(E, 3*E) instead of 3 separate)
      - No rotary embeddings (by design -- inter-chain attention has none)
      - No output projection (output goes through self_attn.out_proj)
      - No incremental state caching (only used during training inference)
      - Always returns (logits, v_heads) -- never does the full softmax path

    The fused QKV reduces kernel launches from 3 to 1 per layer and
    makes each matmul 3x larger for better tensor core utilization.
    """
    ...
    def __init__(
        ...
        # Fused Q, K, V in one weight matrix
        self.fused_qkv = nn.Linear(embed_dim, 3 * embed_dim, bias=bias)
        self._init_parameters()
    ...
    def project_qkv_4d(self, x: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        Uses the fused QKV projection internally. The caller must apply
        scaling and (optionally) RoPE. Multimer attention has no RoPE.
    ...
    def forward(
        ...
        x: Tensor,
        key_padding_mask: Optional[Tensor] = None,
        before_softmax: Optional[bool] = None,
        **kwargs,
    ) -> Tuple[Tensor, Tensor]:
        """Project QKV, compute logits, return (logits, v_heads).

        Args:
            x: Input (T, B, E)
            key_padding_mask: Optional (B, T) bool mask
            before_softmax: Ignored -- always returns pre-softmax
        ...
        qkv = self.fused_qkv(x)                     # (T, B, 3*E)
        q, k, v = qkv.chunk(3, dim=-1)              # each (T, B, E)
        q = q * self.scaling

        # Flatten heads: (T, B, E) -> (B*H, T, D)
        q = q.reshape(T, B * H, D).transpose(0, 1)     # (B*H, T, D)
        k = k.reshape(T, B * H, D).transpose(0, 1)
        v = v.reshape(T, B * H, D).transpose(0, 1)
        ...
        # Reshape for return
        logits = logits.view(B, H, T, T)
        v_heads = v.view(B, H, T, D)

        return logits, v_heads